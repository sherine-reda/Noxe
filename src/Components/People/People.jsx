...
export default function People() {
  ...
  async function getMovies(page) {
    ...
    let { data } = await axios.get(
      `https://api.themoviedb.org/3/person/popular?api_key=643a1195f74a078cff8b3ed415464bed&language=en-US&page=${page}`
    );
                ...
                <Link to={`/details/person/${people.id}`}>
                  <div className="card bg-light overflow-hidden">
                    ...
                    <div className="card-body">
                      <h2 className="h6 card-title fw-bold">{people?.name}</h2>
                    </div>
                  </div>
                </Link>
              </div>
